...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        if np.random.rand() < epsilon:
            ...
            Q_values_current = Q[state, :]
            allowed_Q_values = Q_values_current[all_actions]
            action = all_actions[np.nanargmax(allowed_Q_values)]
        return action
        # ADD YOUR CODE SNIPPET BETWEEN EX 4.1
        ...
        # It is recommended you use the np.random module
        epsilon_generator = ScheduleLinear(
            anneal_timesteps, epsilon_final, epsilon_initial)
        epsilon = epsilon_generator.value(current_total_steps)
        if np.random.rand() < epsilon:
            # Select a random action
            action = np.random.choice(all_actions)
        else:
    ...
    def q_learning(self):
        ...
        while episode <= self.episode_max and diff > self.threshold:
            ...
            while not end_episode:
                ...
                # action = epsilon_greedy(Q, s_current, list_pos)
                action = epsilon_greedy(
                    Q, s_current, list_pos, steps, self.epsilon_initial, self.epsilon_final, self.annealing_timesteps, 'linear')
                # ADD YOUR CODE SNIPPET BETWEEN EX 2.1 and 2.2
                ...
                Q[s_current][action] = (
                    1-lr)*Q[s_current][action] + lr*(R + discount*np.nanmax(Q[s_next, :]))
                # ADD YOUR CODE SNIPPET BETWEEN EX. 2.2
...
class ScheduleLinear(object):
    ...
    def value(self, t):
        ...
        p_diff = self.final_p - self.initial_p
        p_t = self.initial_p + p_diff * t / self.schedule_timesteps
        return p_t
        # ADD YOUR CODE SNIPPET BETWEEN EX 4.2
